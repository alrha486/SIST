...
public class HMS_Main {
   ...
   private Vector<Hospital> vector; // 학생 정보를 저장 할 벡터

   public HMS_Main(String title) {//생성자  
      ...
      this.vector = new Vector<Hospital>(1,1); // 학생 벡터 초기화 
   }
...
//      HMS_Output output = new HMS_Output(this.vector); // 학생 벡터의 주소를 아웃풋 생성자로 보내기
      this.tab.add("입력", new HMS_Input(this.vector, this.tab)); // 입력 택을 추가함과 동시에 인풋 생성자 호출 학생벡터,탭의주소,아웃풋의 주소를 전달
//      this.tab.add("출력", output); // 출력 탭을 추가함과 동시에 아웃풋의 주소를 전달
   ...
   public static void main(String[] args) {
	   HMS_Main main = new HMS_Main("성적관리 프로젝트 Ver.3.0");
      main.display();
